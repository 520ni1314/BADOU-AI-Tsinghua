...
#定义神经网络中间层H1
Weights_L1=tf.Variable(tf.random_normal([200,784]))
biases_L1=tf.Variable(tf.zeros([200,1]))    #加入偏置项
Wx_plus_b_L1=tf.matmul(Weights_L1,x)+biases_L1
L1=tf.nn.tanh(Wx_plus_b_L1)   #加入激活函数

#定义神经网络输出层
Weights_L2=tf.Variable(tf.random_normal([10,200]))
biases_L2=tf.Variable(tf.zeros([10,1]))  #加入偏置项
Wx_plus_b_L2=tf.matmul(Weights_L2,L1)+biases_L2
a_out=tf.nn.tanh(Wx_plus_b_L2)   #加入激活函数
prediction = tf.nn.softmax(a_out)
